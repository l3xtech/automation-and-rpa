...
int main(int argc, char **argv)
{
    ...
    auto opts = opentelemetry::exporter::otlp::OtlpGrpcMetricExporterOptions();
    // opts.url = "http://opentelemetrycollector.core-dev.svc.cluster.local:4318";
    opts.endpoint = argv[1];
    auto grpc_exporter = opentelemetry::exporter::otlp::OtlpGrpcMetricExporterFactory::Create(opts);
    ...
    auto grpcReader = metrics_sdk::PeriodicExportingMetricReaderFactory::Create(std::move(grpc_exporter), options);
...
// #else
    ...
    provider->AddMetricReader(std::move(grpcReader));

    std::string name = "seila-grpc";
...
#ifdef OPENTELEMETRY_DEPRECATED_SDK_FACTORY
    std::shared_ptr<opentelemetry::metrics::MeterProvider> grpc_api_provider(std::move(u_provider));
#else
    std::shared_ptr<opentelemetry::metrics::MeterProvider> grpc_api_provider(std::move(provider));
#endif /* OPENTELEMETRY_DEPRECATED_SDK_FACTORY */

    metrics_api::Provider::SetMeterProvider(grpc_api_provider);
